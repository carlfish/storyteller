...
class RewindCommand(Command):
    ...
    async def run(self, story: Story):
        ...
        if (len(story.current_messages) > 0):
            await self.response.send_message(f"📖 Rewound to last message:\n\n{story.current_messages[-1].content}")
        else:
            await self.response.send_message(f"❗ Already at start of message history.")
...
class RewriteCommand(Command):
    ...
    async def run(self, story: Story):
        ...
        await self.response.send_message(f"📖 Last response rewritten to:\n\n{self.text}")

class SummarizeCommand(Command):
    def __init__(self, chains: Chains, response: Response, min_tokens: int, max_tokens: int):
        self.chains = chains
        self.response = response
    ...
    async def run(self, story: Story):
        if (count_tokens_approximately(story.current_messages) > self.max_tokens):
            ...
            story.scenes = await self.update_scenes(story.scenes, pruned_messages, msg_count)
            story.characters = await self.update_characters(story.characters, pruned_messages, msg_count)
            await self.response.send_message(f"📖 Pruned {len(pruned_messages)} of {msg_count} messages. Scenes {scene_count}→{len(story.scenes)}. Characters {char_count}→{len(story.characters)}.")

class CloseChapterCommand(Command):
    def __init__(self, chains: Chains, summary_response: Response, chapter_response: Response, chapter_title: str):
        self.chains = chains
    ...
    async def close_chapter(self, story: Story):
        ...
        story.scenes = []
        await self.chapter_response.send_message(f"📖 Closed chapter {len(story.chapters)}: {response.title}")
